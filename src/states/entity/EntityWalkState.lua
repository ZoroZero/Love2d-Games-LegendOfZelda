...
function EntityWalkState:update(dt)
    ...
    elseif self.entity.direction == 'down' then 
        self.entity.y = self.entity.y + self.entity.walk_Speed*dt;
        if self.entity.y > MAP_RENDER_OFFSET_Y + (MAP_HEIGHT - 3)*TILE_SIZE + self.entity.height/2 then 
            self.entity.y = MAP_RENDER_OFFSET_Y + (MAP_HEIGHT - 3)*TILE_SIZE + self.entity.height/2;
            self.bumped = true;
        end
    -- left movement
    ...
    elseif self.entity.direction == 'right' then 
        self.entity.x = self.entity.x + self.entity.walk_Speed*dt;
        if self.entity.x > MAP_RENDER_OFFSET_X + (MAP_WIDTH -2) * TILE_SIZE then 
            self.entity.x = MAP_RENDER_OFFSET_X + (MAP_WIDTH -2) * TILE_SIZE;
            self.bumped = true;
        end
    end
end
